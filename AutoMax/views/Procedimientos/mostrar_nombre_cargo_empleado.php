...
if (isset($_GET['id'])) {
    ...
    oci_bind_by_name($stid, ':fecha_contratacion', $fecha_contratacion, 50);
    oci_bind_by_name($stid, ':salario', $salario, 20);
    oci_execute($stid);
    oci_free_statement($stid);

    // Llamar al procedimiento mostrar_nombre_cargo_empleado
    $emp_id = $id_empleado; // Usar el ID del empleado proporcionado en la URL
    $sql_empleado = 'BEGIN mostrar_nombre_cargo_empleado(:emp_id); END;';
    $stid_empleado = oci_parse($conn, $sql_empleado);
    oci_bind_by_name($stid_empleado, ':emp_id', $emp_id);

    // Ejecutar el procedimiento y capturar la salida de DBMS_OUTPUT
    ...
    oci_free_statement($stid_empleado);
    oci_close($conn);
} else {
