...
if (isset($_GET['cod_factura']) && isset($_GET['cod_repuesto']) && isset($_GET['cantidad'])) {
    ...
    if ($r) {
        // Redirigir a la página de detalles de facturas después de la inserción exitosa
        header('Location: Tabla_Detalle_Factura.php');
        exit(); // Asegura que el script se detenga después de la redirección
    } else {
        $e = oci_error($stid);
        echo "<p>Error al crear el detalle de factura: " . htmlentities($e['message'], ENT_QUOTES) . "</p>";
    }
