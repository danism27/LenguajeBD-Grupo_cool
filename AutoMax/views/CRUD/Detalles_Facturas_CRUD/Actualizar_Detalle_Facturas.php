...
if (isset($_GET['id'])) {
    ...
    $sql = 'BEGIN get_detalle_factura(:id_detalle, :cod_factura, :cod_repuesto, :cantidad); END;';
    $stid = oci_parse($conn, $sql);

    // Bind del parámetro
    oci_bind_by_name($stid, ':id_detalle', $id_detalle);
    oci_bind_by_name($stid, ':cod_factura', $cod_factura, 50);
    oci_bind_by_name($stid, ':cod_repuesto', $cod_repuesto, 50);
    oci_bind_by_name($stid, ':cantidad', $cantidad, 50);
...
<head>
    <title>Actualizar Detalle de Factura - Paso 1</title>
    <meta charset="utf-8">
...
<body>
    ...
    <div class="container">
        <h1 class="text-center">Actualizar Detalle de Factura</h1>
        <!-- Formulario para ver y enviar los datos del detalle de factura -->
        <form action="Procesar_Actualizar_Detalle_Factura.php" method="GET">
            <!-- Campo oculto para el ID del detalle de factura -->
            <input type="hidden" name="id_detalle" value="<?php echo htmlentities($id_detalle, ENT_QUOTES); ?>">

            <div class="form-group">
                <label for="cod_factura">Código de Factura:</label>
                <input type="text" class="form-control" id="cod_factura" name="cod_factura"
                    value="<?php echo htmlentities($cod_factura, ENT_QUOTES); ?>" required>
            </div>

            <div class="form-group">
                <label for="cod_repuesto">Código de Repuesto:</label>
            ...
            <div class="form-group">
                ...
                <input type="number" class="form-control" id="cantidad" name="cantidad"
                    value="<?php echo htmlentities($cantidad, ENT_QUOTES); ?>" required>
            </div>

            <br><br>
            <button type="submit" class="btn btn-primary">Actualizar</button>
            <a class="btn btn-secondary" href="Tabla_Detalles_Factura.php">Volver</a>
        </form>
    </div>
