    ...
    <header style="margin-bottom: 2%">
        ...
        <div class="container">
            ...
            <div class="mb-3 d-flex justify-content-center align-items-center">
                <!-- Mini Formulario para Leer Vehículo Específico -->
                <form action="Leer_Vehiculo.php" method="GET" class="form-inline d-flex mr-3">
                    <label for="id_vehiculo" class="mr-2">Buscar Vehículo por ID:</label>
                    <input type="number" id="id_vehiculo" name="id_vehiculo" class="form-control mr-2" required>
                    <button type="submit" class="btn btn-primary">Leer</button>
                </form>
                <button type="button" class="btn btn-success ml-3"
                    onclick="window.location.href='Crear_Vehiculos.php'">Crear</button>
            </div>
            ...
            <div class="row">
                <div class="col">
                    ...
                    $stid = oci_parse($conn, 'SELECT * FROM VEHICULOS ORDER BY ID_VEHICULO ASC');
                    oci_execute($stid);
                    ...
                    while ($row = oci_fetch_array($stid, OCI_ASSOC + OCI_RETURN_NULLS)) {
                        ...
                        echo "<td>" . htmlentities($row['MARCA'], ENT_QUOTES) . "</td>\n";
                        echo "<td>" . htmlentities($row['MODELO'], ENT_QUOTES) . "</td>\n";
                        echo "<td>" . htmlentities($row['FECHA_REGISTRO'], ENT_QUOTES) . "</td>\n";
                        echo "<td>" . htmlentities($row['NOMBRE_USUARIO'], ENT_QUOTES) . "</td>\n";
                        echo "<td><a href='Actualizar_Vehiculo.php?id=" . $row['ID_VEHICULO'] . "' class='btn btn-warning btn-sm'>Actualizar</a></td>\n";
                        echo "<td><a href='Procesar_Eliminar_Vehiculo.php?id=" . $row['ID_VEHICULO'] . "' class='btn btn-danger btn-sm'>Eliminar</a></td>\n";
                        echo "</tr>\n";
                    }
